...
let NJ = 8
    ...
    function D(f, n = 1)
        for _ in 1:n
            f = expand(sum(fam[k+2] * diff(f, fam[k+1]) for fam in FAM for k in 0:(NJ-2)))
        end
        f
    end
    v(k) = vj[k+1]; w(k) = wj[k+1]
    ...
    vt = expand(D(dHdv))
    check("v_t = d_x ( 2vu - u_x ) = 6 v^2 v_x - v_xxx, i.e. mKdV under the constant structure",
...
function miura_bracket_data(n; mean = 1.0, p = 3)
    ...
    dDdv = [-2 * sum(Minv[a, b] * T[j, b, c] for b in 1:nb) for j in 1:nb, a in 1:nb, c in 1:nb]
    dPdv = [sum(dDdv[j, a, c] * P1[c, d] * DM[b, d] + DM[a, c] * P1[c, d] * dDdv[j, b, d]
                for c in 1:nb, d in 1:nb) for j in 1:nb, a in 1:nb, b in 1:nb]
    DMinv = inv(DM)
    dPdu = [sum(DMinv[j, m] * dPdv[j, a, b] for j in 1:nb) for m in 1:nb, a in 1:nb, b in 1:nb]
...
let resid = Float64[]
    ...
    check("the Jacobi residual is at machine precision at every N, against 0.42 -- flat -- " *
          "for the Galerkin P^2", maximum(resid) < 1e-11,
...
let n = 24, r = miura_bracket_data(n)
    ...
    @printf("      circular bandwidth, Miura K_M:   %3d   (n = %d, so n/2 = dense)\n",
            band(KM), n)
    check("K_M is dense while the Galerkin assembly is local: the two interior Minv are " *
...
let prev = nothing, prev_n = 0, orders = Float64[]
    ...
    check("on smooth data the two discretisations converge to one another",
          minimum(orders) > 0.8,
          "observed orders [" * join(["'" * (@sprintf("%.1f", o)) * "'" for o in orders], ", ") * "]")
end
...
let ent = Float64[]
    ...
    check("the entrywise difference stays of order one -- the same signature as the " *
          "Jacobi violation, and in the same modes", minimum(ent) > 0.1,
...
let c = Chart(20; p = 3, v0 = x -> 1.0 + sin(x))
    ...
    check("C_0,d = int u_h = -int v_h^2, exactly -- so the image of M_h lies in " *
          "C_0,d <= 0", abs(lhs - rhs) / abs(lhs) < 1e-14,
...
let found = Dict{Float64,Vector{Float64}}()
    ...
    check("a preimage exists exactly when the discrete Hill operator is positive",
          all(!isempty(found[cc]) == (hill_lambda0(s20, ubase(cc)) > 0) for cc in keys(found)),
          string(Dict(k => !isempty(v) for (k, v) in found)))
    check("and where it exists there are two, the two Floquet solutions, with " *
          "opposite int v -- so M_h is not injective either",
          all(length(v) == 2 && abs(v[1] + v[2]) < 1e-6 for v in values(found) if !isempty(v)),
          string(found))
end
...
for (lab, f, Lx, n) in EXAMPLES
    ...
    u = project(se, f)
    ok = any(miura_invert(se, u; seed = sd) !== nothing for sd in range(-4.0, 4.0; length = 17))
...
let c = Chart(20; p = 3, v0 = x -> 1.0 + sin(x))
    ...
    check("the second solve is z_h = Pi( 2 v_h u_h - u_h,x ) = Pi( 2 v^3 - v_xx ) " *
          "to the discretisation error", e < 1e-3, @sprintf("rel. difference %.2e", e))
...
let plain = Dict{Int,Float64}()
    ...
    check("while the plain form is a different system until the quadrature " *
          "integrates d_x(phi_k phi_l) exactly", plain[2] > 1e-4 && plain[3] < 1e-12,
...
check("dC_0,d/dt does not vanish identically: it is order one on a broadband " *
...
let rng = MersenneTwister(0), DT = 8e-3
    ...
    check("positive control: a midpoint step commutes with an AFFINE change of " *
          "variables, to round-off", e < 1e-13, @sprintf("%.1e", e))
end
...
let prev = nothing, rs = Float64[]
    ...
    check("the two differ at O(dt^3) per step: the Miura map is quadratic, and " *
...
let DT = 8e-3, pois = Dict{String,NTuple{2,Float64}}()
    for (name, stepper) in (("implicit midpoint", midpoint_v), ("discrete gradient", dgrad_v))
        y, Dy = stepper(cc9, vd0, DT)
        if Dy === nothing                       # central difference, as elsewhere
            Dy = zeros(cc9.n, cc9.n); eps = 1e-6
            for j in 1:cc9.n
    ...
    check("implicit midpoint in v pushes forward to a genuine Poisson integrator " *
          "of the Miura bracket -- the first one the notes have for the second " *
          "KdV structure", pois["implicit midpoint"][2] < 1e-11,
...
let (Ku, K0) = K2_blocks(cc9.s)
    ...
    check("both Galerkin brackets keep the mass to round-off on this data, by " *
          "eq. (mass-casimir)",
          maximum(got[n][3] for n in keys(got) if !occursin("Miura", n)) < 1e-11,
          string(Dict(n => @sprintf("%.1e", v[3]) for (n, v) in got if !occursin("Miura", n))))
    check("implicit midpoint does NOT conserve H_2,d in the v chart, where it is " *
          "quartic", got["midpoint, Miura"][2] > 1e-6,
          @sprintf("%.1e", got["midpoint, Miura"][2]))
    check("the discrete gradient does, exactly -- the value of a function is " *
          "chart-free", got["discrete gradient, Miura"][2] < 1e-11,
          @sprintf("%.1e", got["discrete gradient, Miura"][2]))
    check("both Miura runs lose the mass, which no other method-flow pair in the " *
          "notes does",
...
let nt = 125, dts = 1.6e-3
    ...
    check("one equation, one method, two charts, two answers -- H_2,d is quadratic " *
...
let (Ku, K0) = K2_blocks(cc9.s)
    ...
    check(@sprintf("in v the error in H_2,d is a clean O(dt^2), fitted rate %.2f",
                   sum(r) / length(r)), 1.9 < sum(r) / length(r) < 2.1,
          "rates [" * join(["'" * (@sprintf("%.2f", x)) * "'" for x in r], ", ") * "]")
    check("in u it sits at round-off at every step size, H_2,d being quadratic " *
          "there and an exact invariant", maximum(s[3] for s in sweep) < 1e-12,
    ...
    check("the discrete gradient adds a bounded O(dt^2) to the mass where the " *
    ...
    check("the mass error, by contrast, is a dt-independent plateau: the time " *
